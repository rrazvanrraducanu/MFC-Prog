...
CProgDlg::CProgDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_PROG_DIALOG, pParent)
	, m_strProg(_T(""))
{
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_CBString(pDX, IDC_COMBO1, m_strProg);
...
BOOL CProgDlg::OnInitDialog()
{
	...
	m_ctrlCombo.AddString(_T("Notepad"));
	m_ctrlCombo.AddString(_T("Paint"));
...
void CProgDlg::OnBnClickedButton1()
{
	...
	UpdateData(TRUE);
	CString prg = m_strProg;
	LPCSTR prg1 = (LPCSTR)(LPCTSTR)prg;
	prg.MakeUpper();
	if (prg == _T("PAINT"))
		WinExec("mspaint.exe", SW_SHOW);
	else if (prg == _T("NOTEPAD"))
		WinExec("notepad.exe", SW_SHOW);
	else WinExec(prg1, SW_SHOW);
}
